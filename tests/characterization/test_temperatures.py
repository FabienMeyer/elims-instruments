...
def test_temperature_measurement_is_optional() -> None:
    """Measurement fails clearly when no getter is configured."""
    temperature = Temperature(temperature_specification())

    with pytest.raises(NotImplementedError, match="getter is not provided"):
...
def test_temperature_enable_and_disable_use_callbacks() -> None:
    """Temperature source power operations delegate to their callbacks."""
    events: list[str] = []
    temperature = Temperature(
        temperature_specification(),
...
def test_temperature_power_control_requires_callbacks() -> None:
    """Unsupported source controls fail with temperature-specific messages."""
    temperature = Temperature(temperature_specification())

    with pytest.raises(NotImplementedError, match="Cannot enable temperature 'DUT'"):
        temperature.enable()
    with pytest.raises(NotImplementedError, match="Cannot disable temperature 'DUT'"):
        temperature.disable()


def test_temperature_preserves_setpoint_when_hardware_write_fails() -> None:
    """A failed hardware write does not change the configured setpoint."""

    def failing_setter(_value: float) -> None:
        raise RuntimeError("chamber communication failed")

    temperature = Temperature(
        temperature_specification(
            temperature_limits=Limits(typical=25, minimum=-40, maximum=125)
        ),
        temperature_setter=failing_setter,
    )
