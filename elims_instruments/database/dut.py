...
class DutCrud(Crud[DutModel]):
    ...
    def add(self, data: DutModel) -> DutModel:
        """Validate a table-model instance before persisting it."""
        validated = DutModel.model_validate(data.model_dump())
        return super().add(validated)

    def upsert_many(self, records: Sequence[DutModel]) -> tuple[int, int]:
        """Validate every table-model instance before applying the batch."""
        validated = [
            DutModel.model_validate(record.model_dump()) for record in records
        ]
