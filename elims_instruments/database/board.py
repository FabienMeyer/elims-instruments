...
class BoardCrud(Crud[BoardModel]):
    ...
    def add(self, data: BoardModel) -> BoardModel:
        """Validate a table-model instance before persisting it."""
        validated = BoardModel.model_validate(data.model_dump())
        return super().add(validated)

    def upsert_many(self, records: Sequence[BoardModel]) -> tuple[int, int]:
        """Validate every table-model instance before applying the batch."""
        validated = [
            BoardModel.model_validate(record.model_dump()) for record in records
        ]
